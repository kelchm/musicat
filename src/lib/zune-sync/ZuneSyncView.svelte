<script lang="ts">
    ...
    onMount(async () => {
        // Load local songs (MP3 only for this PoC)
    ...
    async function detectDevices() {
        ...
        try {
            ...
            if (devices.length === 0) {
                statusMessage = "No MTP devices found. Is your Zune connected and unlocked?";
            } else {
    ...
    function toggleSongSelection(songId: string) {
        ...
        selectedSongIds = selectedSongIds; // trigger reactivity
    }
    ...
    async function sendSelectedToDevice() {
        if (selectedDeviceIndex === null || selectedSongIds.size === 0) return;
        sending = true;
        error = null;
        ...
        if (!error) {
            ...
            selectedSongIds = new Set();
            // Refresh device tracks to see the new ones
            await loadDeviceTracks();
        }
...
<div class="zune-sync-view">
    <header>
        ...
        <p class="subtitle">Proof of concept — MTP device sync</p>
    </header>
            ...
            <div class="device-list">
                {#each devices as device, i}
                    <button
                        ...
                        on:click={() => selectDevice(i)}
                    >
                        <div class="device-name">
                            {device.friendlyName || device.modelName || device.product || "MTP Device"}
                        </div>
                    ...
                    <table>
                        ...
                        <tbody>
                            {#each deviceTracks as track}
                                <tr
                                    class:has-plays={track.playCount > 0}
                                >
                                    <td class="col-title">{track.title}</td>
                                    <td class="col-artist">{track.artist}</td>
                                    <td class="col-album">{track.album}</td>
                                    <td class="col-duration"
                                        >{formatDuration(track.durationMs)}</td
                                    >
                                    <td class="col-plays">
                                        {#if track.playCount > 0}
                                            <span class="play-count"
                                                >{track.playCount}</span
                                            >
                                        {:else}
                                            —
                                        {/if}
                ...
                <div class="header-actions">
                    ...
                    >
                        {#if sending}
                            Sending...
                        {:else}
                            Send {selectedSongIds.size} Track{selectedSongIds.size !==
                            1
                                ? "s"
                                : ""}
                        {/if}
            ...
            {#if sending && transferProgress}
                <div class="progress-bar-container">
                    ...
                    <div class="progress-detail">
                        {formatSize(transferProgress.bytesSent)} / {formatSize(
                            transferProgress.bytesTotal
                        )}
                    ...
                    <table>
                        ...
                        <tbody>
                            {#each localSongs as song}
                                <tr
                                    class:selected={selectedSongIds.has(
                                        song.id
                                    )}
                                    on:click={() =>
